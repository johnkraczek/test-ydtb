...
import { useSidebar } from "@ydtb/core/context/sidebar/use-sidebar";
...
export function ToolSidebar() {
  ...
  return (
    <>
      <div
        className={`bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl h-full rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${isOpen ? "w-64 opacity-100 translate-x-0" : "w-0 opacity-0 -translate-x-4 overflow-hidden border-0 ml-0"
          }`}
      >
      ...
      {/* Add right spacing when sidebar is open */}
      {isOpen && <div className="w-2" />}
    </>
  );
}
