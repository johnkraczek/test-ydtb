                  ...
                  {column.items.map(item => {
                    ...
                    return (
                      <ContextMenuWrapper key={item.id} item={item}>
                        <DraggableItem item={item}>
                          <div
                            className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer text-sm ${isLeafSelected
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {currentItems.map(item => (
          <ContextMenuWrapper key={item.id} item={item}>
            <DraggableItem item={item}>
              <div
                className={`group relative flex flex-col gap-2 p-3 rounded-xl border transition-all duration-200 cursor-pointer hover:shadow-md ${selectedItems.includes(item.id)
              ...
              >
                <div className="aspect-square w-full rounded-lg overflow-hidden bg-slate-50 dark:bg-slate-950 flex items-center justify-center relative">
                  ...
                  <div className={`absolute top-2 right-2 h-5 w-5 rounded-full border flex items-center justify-center transition-opacity ${selectedItems.includes(item.id)
